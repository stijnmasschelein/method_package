...
ipw <- function(data, weights){
  weights <- weights/sum(weights)*nrow(data) 
...
boot_ipw <- bayesboot(sim_data, ipw, R = 100, 
                      use.weights = TRUE)
fwb_ipw <- fwb(sim_data, ipw, R = 100)
summary(boot_ipw)
summary(fwb_ipw)
```
...
fwb_function <- function(data){
  fwb <- fwb(data, ipw, R = 200, verbose = FALSE)
  se <- summary(fwb)["x", 2]
  return(se)
}
fwb_function(sim_data)
sim <- sim_data %>%
  nest(.by = sim) %>%
  mutate(se = future_map_dbl(.x = data, .f = ~ fwb_function(.x),
