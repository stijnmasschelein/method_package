...
knitr::opts_chunk$set(echo = FALSE, dpi = 200, dev = "svg",
                      fig.height = 5, fig.width = 10)
library(tidyverse)
library(cowplot)
library(viridis)
library(here)
ttheme_set(theme_cowplot())
options(tibble.width = 60)
options(width = 60)
```

```{r xaringan-themer, include=FALSE, warning=FALSE}
# This code block is to mimic the UWA approved style
uwa_blue = "#003087"
uwa_gold = "#DAAA00"
library(xaringanthemer)
style_duo_accent(
    primary_color = uwa_blue, #UWA blue
    secondary_color = uwa_gold,  #UWA gold
...
???
The emphasis on the units will be on the practical and statistical issues of the data analysis part of a thesis. The influence will not be on the accounting and finance part of your thesis. The goal is to be relevant to everyone in the unit.
...
- (Almost weekly) Assignments (70%)
    ...
    - Assignment 2 (7.5%) (TBA)
    - Assignment 3 (7.5%) (TBA)
    - Assignment 4 (7.5%) (TBA)
...
???
We want you to (1) do some data analysis and (2) be well prepared to undertake (the data analaysis part of) a research project. So we are going to evaluate you by letting you (1) analyse data and (2) prepare your honours thesis.
...
folder = "~/Dropbox/Teaching/lecturenotes/method_accfin/data/"
...
eqn_comp = substitute(italic("compensation") == a * italic("MV") ^ b,
                      list(a = as.numeric(format(coef(power_law_comp)[1], digits = 2)),
                           b = as.numeric(format(coef(power_law_comp)[2], digits = 2))))
# create the plot 
plot_comp_value =  
  ...
  stat_function(fun = 
                ...
                colour = "blue") +
  # Add a title
  ggtitle("US CEO compensation 2011-2019") +
...
???
The data is downloaded from Compustat and Execucomp. A lot of you
will use these are similar databases in your research project. I
did not clean or check the data for this exercise. In your own
project, you should show a better understanding of how the data
are gathered and what they include than what I am displaying
here.

- CEO compensation is fairly complete. It includes changes in the
  value of equity and options.
- Market value also includes all outstanding financial
  instruments on the company.
...
coefficient is 0.23. Prior studies have found a coefficient more
...
???
Note what we have just done. We started with some assumptions
about the production function of a company and competitive
markets to find the theoretical relation between firm size and
CEO compensation. We followed up by testing this theory to data
from S&P500 firms. These are the steps that you should be
following.
...
interesting new research questions. I am not sure whether my
strategy will work for you. I find that a good review paper gives
a good list of papers you can build on and they often already
compare the most important papers in a field. The trick is to be
not too picky. You probably will not find a review for your exact
reserch problem but it is unlikely that you will not find a
partly relevant overview paper. You can search for review papers
by adding "intitle:review" or "intitle:overview" to your Google
Scholar search.
...
To find good reviews, I think you should start your search in the
better journals. Some journals are dedicated to these literature
reviews for instance *Journal of Economic Literature* and
*Journal of Accounting Literature*. I am not aware of a similar
journal in finance but I will happily add it if you let me know.

When you start your literature search, you don't want to narrow.
You are not going to find an overview paper about "CEO
compensation in Australian mining companies after the GFC".
However, you can start with an overview paper about CEO
compensation. Like the one I found:  ["Executive Compensation: A
Modern Primer"](https://dx.doi.org/10.1257/jel.20161153) by Alex
Edmans and Xavier Gabraix in *Journal of Economic Literature*.
