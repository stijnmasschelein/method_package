...
We are using the `tidymodel` package for the application and I will follow the [introduction to tidymodels](https://www.tidymodels.org/start/). The advantage of the tidymodels approach is that you can follow a very similar workflow for other machine learning methods than the one I will be showing here. The code itself is not that important. The goal is more to give you a starting point.
