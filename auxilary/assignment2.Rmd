...
3. *Regressions*
   ...
   - Create a regression, `cheap_lm1`, with `observed return` as the dependent variable and `csr_report` as the independent variable.
...
3. *Regressions*
    ...
    ```{r regression-a2a1, results = 'asis', eval = eval_assignment2 }
...
```{r, simulation-iv, eval = eval_assignment2}
...
2. Create the following *regressions*:
   ...
   - I show how you can get the first stage and second stage regression of the `iv` object to incorporate them into the `msummmary` output.
...
2. *Regressions*
    ...
    iv_lm <- lm(..., data = sig_iv)
    iv_lm_reduced <- lm(..., data = sig_iv)
    iv <- feols(return ~ 1 | ... , data = sig_iv)
