...
3. *Regression*: Run a linear regression with `observed_return` as the dependent variable and `donated` as the independent variable. Use `summary` to show the results of the regression.
...
sig <- tibble(
  ...
  mutate(
    ...
    donated = ifelse(observed_donation == 1, "yes", "no")
  )
glimpse(sig)
```
...
sig_plot <- ggplot(sig, aes(x = donated, y = observed_return)) +
    geom_jitter(width = .3)
plot(sig_plot)
```
...
2. *Plotting*: Make a similar plot as for the signalling model and call it `cheap_plot`. Whether a company has an observed csr reported should be on the x-axis and the observed returns should be on the y-axis.
...
cheap_plot <- ggplot(cheap, aes(x = reported,
                                y = observed_return)) +
geom_jitter(width = .3)
plot(cheap_plot)
```
...
```{r cheap-msummary, eval = eval_assignment1}
...
2. *Plot*
   - Make a figure with the density of `observed_return` for the observations who `donated` and who did not in a different colour.

3. *Regression*
   - Run a regression with `observed_return` as dependent variable and donation as independent variable.
   - Run a regression with `observed_bid` as dependent variable and donation as independent variable.
...
```{r, simulation-assignment2-cross, eval = eval_assignment2}
...
sig2 <- tibble(
    ...
    observed_donation = observed_donation,
    observed_bid = observed_bid) %>%
    mutate(donated = ifelse(observed_donation == 1, "yes", "no"))
```

2. *Plot*

```{r plot-assignment2-cross, eval = eval_assignment2}
plot_sig2 <- ggplot(sig2, aes(colour = donated, x = observed_return)) + 
    geom_density()
plot(plot_sig2)
```
...
```{r regression-a2a1, eval = eval_assignment2}
sig_lm_return <- lm(observed_return ~ observed_donation, data = sig2)
sig_lm_bid <- lm(observed_bid ~ observed_donation, data = sig2)
...
```{r, simulation-it-quality, eval = eval_assignment2}
...
2. Run the following four *regressions* and show the results with the
   ...
   - `observed_return` as dependent variable and `observed_donation` as independent variable.
   ...
   - Add `observed_performance` as a control variable to the second regression.
   - Change the dependent variable to `observed_bid` in the first regression.
...
```{r, fig.width = 5, fig.height = 3, eval = eval_assignment2}
...
```{r, results = 'asis', eval = eval_assignment2}
sig_lm1 <- lm(observed_return ~ observed_donation, data = sig3)
sig_lm2 <- lm(observed_return ~ observed_donation + high_quality,
              data = sig3)
sig_lm3 <- lm(observed_return ~ observed_donation + high_quality
              + observed_performance,
            data = sig3)
sig_lm4 <- lm(observed_bid ~ observed_donation, data = sig3)
...
- In my simulation both regression (1) and (2) give a nonsignificant effect at the 5% level but if you would crank up the number of observations (i.e. `N <- 10000`), you would find a significant effect in regression (1). In my case the coefficient is positive and that is what I meant to have. In summary, the correct answer here is that we need to report regression (2) because `high_quality` is a confounder as we can see from the causal diagram.

- Regression (4) gives us the result that we want because we can isolate the bid effect from the performance effect. That is return (measured by $b - 0$ in regression (4)) that we observe is driven by the donation and not by investors anticipating the performance. You can see that the coefficient for donation is very similar in (3) and (4) but with the opposite sign. The switch in the sign is because in our calculation of return, $b$ has a positive sign in regression (4) but a negative sign in (3), i.e. $p - b$. The reason why regression (3) works is that the variable `performance` also functions as a confounder and we recover the correct estimate once we adjust for `performance`. You can use the causal diagram below to see for yourself that `performance` is a confounder.
...
fix_forget <- feols(actual_payoff ~ actual_donation | firm,
                    data = rd_panel_forget)
...
```{r, eval = eval_assignment3}
fix_mistake <- feols(actual_payoff ~ actual_donation | firm,
                     data = rd_panel_mistake)
...
In both regressions, we use firm fixed effects which means that the variation in the dependent variable, `actual_donation`, stems from firms that sometimes donate and sometimes do not donate. In the `forget` data, this variation is only from the high performing firms who benefit ($1$) from donating. In the `mistake` data, this variation stems from the 50% high performing firms who benefit ($1$) and from the 50% low who suffer from donating ($-4-2$). The average of those two groups is $-3.5$ and is only 1 standard error away from the estimate in the `mistake` regression.

## Instrumental Variable Approach

### Identification by Local Elections

In this case, we are going to create data where we assume that when there is a local election, firms will not donate. They are afraid that it will be seen as political and that it will backfire. For every observation in the data there is a 1/3 probability of a local election. There are 2000 firms in the data and they only appear once each. The only effect of local elections is on the probability of an actual donation, it has no further effect on the payoff of different firms. We can use the `local_election` as an instrumental variable to identify some of the variation in the decision to donate.
...
As before, because we use a fixed effects model, the variation only comes from firms that change their donation decision in the sample. In this case, only high performing firms will change their decision based on whether there is an election or not and we should get an estimate of the effect of donations around 1. The instrumental variable approach gives us the correct estimate in contrast to the OLS estimate. We can see from the first stage that the instrumental variable is relevant because local elections do decrease the probability of a donation (by 0.5 or for half the sample!). We can also see that as a result of the local election the payoff goes down by about -0.38 which comes from a decrease in the ATT of 1 for half of the firms (Notice the standard error!). 
...
The natural disaster identifies variation in the firms that should not donate and have a decrease in payoff of -6 if they do donate. This is supported by the instrumental variable approach. The natural disasters decrease overall payoff by -3 according to the reduced form specification and increase the probability of donating by 0.5 (i.e. for the low performing firms).
