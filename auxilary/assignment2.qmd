...
return  <- ...
observed_return <- ...
observed_donation <- ifelse(rbinom(N, 1, .x) == 1,
                            ...)
sig2 <- tibble(
...
plot_sig2 <- ggplot(.x, aes(colour = .y, x = .z)) +
   ...
plot(plot_sig2)
```
...
sig_lm_return <- lm(..., data = .x)
sig_lm_bid <- lm(..., data = .x)
