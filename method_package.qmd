...
The [fourth](slides/slides4.qmd){target="_blank" rel="noopener noreferrer"} and [fifth](slides/slides5.qmd){target="_blank" rel="noopener noreferrer"} set of slides basically give up on trying to control for everything. The goal is to focus on the research design, i.e. find a situation where we can be reasonable sure that our research question is answerable. I will focus my attention on event studies and its bigger (but slower) brother difference-in-difference and instrumental variables.
